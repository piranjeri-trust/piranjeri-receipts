...
def load_donors():
    ...
    # This avoids pandas read_csv complaints about inconsistent column counts.
    records = []
    for line in raw_lines:
        ...
        name = parts[0].strip()
        mobile = parts[1].strip() if len(parts) > 1 else ""

        # Skip header rows (any line where the name field is a known header word)
...
def save_donors(df):
    # Ensure mobile numbers are saved as clean strings, never as floats
    df = df.copy()
    df["Mobile Number"] = df["Mobile Number"].astype(str).str.replace(r'\.0$', '', regex=True).replace("nan", "")
