...
def ensure_donor_file():
    if not DONOR_FILE.exists():
        DONOR_FILE.write_text("NAME,Mobile Number\n", encoding="utf-8")

def _write_donors_csv(df):
    """Write donors.csv as plain text — never via pandas to_csv —
    Mobile numbers are written in double quotes so pandas always reads
    them back as strings, never as floats. This permanently prevents .0.

    Example of what gets written:
        NAME,Mobile Number
        Ananatha Subramanian Raju,"914443673183"
        No Mobile Donor,""
    """
    ...
    for _, row in df.iterrows():
        ...
        # Strip any .0 that may have crept in from pandas float conversion
        if mobile.endswith(".0"):
            mobile = mobile[:-2]
        if mobile.lower() in ("nan", "none"):
            mobile = ""
        if name and name.lower() not in ("nan", "none", ""):
            # Quote the mobile number so pandas reads it as a string on next load
...
def load_donors():
    ...
    # ── Parse line by line — handles mixed 1-col and 2-col rows ───
    # Never uses pandas to parse — so mobile numbers stay as strings always.
    records = []
    for line in raw_lines:
        ...
        mobile = parts[1].strip().strip('"').strip("'") if len(parts) > 1 else ""

        # Skip header rows
        if name.lower() in header_markers:
            continue
        # Skip blank names
        if not name or name.lower() == "nan":
            continue
        # Strip .0 at parse time — belt and braces
        if mobile.endswith(".0"):
            mobile = mobile[:-2]
        if mobile.lower() in ("nan", "none"):
            mobile = ""

        records.append({"NAME": name, "Mobile Number": mobile})
    ...
    # Remove .0 suffix that appears when numbers are read as floats (e.g. 914443673183.0 → 914443673183)
    df["Mobile Number"] = df["Mobile Number"].str.replace(r'\.0$', '', regex=True)
    # Treat 'nan' mobile as empty
    df["Mobile Number"] = df["Mobile Number"].replace("nan", "")

    return df[["NAME", "Mobile Number"]]

def save_donors(df: pd.DataFrame):
    _write_donors_csv(df)
